...
export function detectLocale(): Locale {
  if (typeof window === 'undefined') return 'pt-BR'; // matches the statically-rendered HTML
  try {
 ...
 * pt-BR (matching the statically-rendered HTML) and reads this value in a
 ...
 */
export const INLINE_DETECT_SCRIPT = `(function(){try{var k='${STORAGE_KEY}';var s=localStorage.getItem(k);var v=null;if(s==='pt-BR'||s==='en'||s==='es'){v=s;}if(!v){var langs=(navigator.languages&&navigator.languages.length)?navigator.languages:[navigator.language||'en'];for(var i=0;i<langs.length;i++){var l=(langs[i]||'').toLowerCase();if(l.indexOf('pt')===0){v='pt-BR';break;}if(l.indexOf('en')===0){v='en';break;}if(l.indexOf('es')===0){v='es';break;}}}if(!v)v='en';window.__Z_LANG__=v;}catch(e){}})();`;
